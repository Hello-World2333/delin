...
1. **"让出"用 `os.msleep(0)`，判据是"累计了多少 CPU 时间"而不是次数**，项目的统一写法是
   ...
   **为什么"HSE 让出只要 2ms"这句话在进程侧不成立（实测数据，别照抄文档）**：`cc_hse.ko`
   的 `os.msleep(0)` 走 `waitTicks(4)` → 4 次 `hse.waitNextTick()`；外设本身是好的
   （模块 init 里直测：`freq=2000`，`setFrequency/setPushEvents/setOn` 之后 10 次
   `waitNextTick` 只用 5ms —— 注意**要先把累加器排空**，否则第一次调用会立刻返回、
   把测量骗成"0.5ms/tick"）。但从**进程**（也就是所有工具与 shell）调用同一条路径时，
   每次 `os.msleep` 都要 ~50ms。两次实测（x50/x20）都一致，所以不是噪声。
   想真拿到亚刻让出，得从 mod 侧解决（例如让 `waitNextTick` 不经过"等下一个游戏刻"
   那条路，或者用低频率的 push 模式 + 内核定期排空），**不是把工具的时间片调小**。
