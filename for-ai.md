...
1. **"让出"用 `os.msleep(0)`，判据是"累计了多少 CPU 时间"而不是次数**，项目的统一写法是
   ...
   **最终做法：一个独立泵进程 + 到点唤醒**（用户给的方案，已落地为 `cc_hse.ko` v0.5.0）：

   - **泵**：内核在**第一次有人要睡时** spawn 一个独立进程 `hse-pump`，**只有它**调 `waitNextTick()`，
     把拍累加进一条**全系统共享的单调时钟**，每推进一次就 `queueEvent("hse_wake")`；没有等待者时
     它 `os.sleep(0.05)` 歇着（既不烧 CPU，也不会像 2kHz 推模式那样淹 256 事件队列）。
   - **等待者**：进程侧 `os.msleep(ms)` 只算自己的 deadline，然后 `pulledEvent("hse_wake")` 让出，
     醒来重新判定（condition-variable 风格）。**进程侧永不碰那个外设计数器**，所以没有互斥、
     没有互相偷拍，所有等待者同等精度。

   实测（电脑 #3，干净镜像；10 次 `os.msleep(0)` 的累计耗时）：

   | 场景 | v0.3.0（各自去等 + 单泵兜底） | v0.5.0（专用泵进程） |
   |---|---|---|
   | 单进程 | 6–8 ms/次 | ~24 ms/次 |
   | 三个进程同时 | 49–51 ms/次（只有一个精确） | **~29 ms/次，三者完全同步** |
   | 并发过去之后单进程 | 49 ms（**粘性**） | ~29 ms（无粘性） |

   为什么 v0.5.0 单进程反而慢：多了一跳「等待者 → 泵(等拍) → 事件 → 等待者」。实测**每多一次
   调度器往返就多 ~6–10ms** —— 这也解释了为什么**进程侧根本拿不到 2ms 级的睡眠**（那需要 0 跳）。
   v0.5.0 换来的是**一致性**：谁都不吃亏，也没有 0.3.0 那种"并发之后整机钉在 50ms"的粘性。

   两个坑（都真机踩过，都已修）：
   1. **模块 init 里 spawn 会抢走 pid 1**（实测进程表 `1:hse-pump 2:init`）：模块装载早于 init，
      所以内核自己起的进程必须**延迟到 init 之后**（v0.5.0 改成"第一次有人睡时才 spawn"，
      ppid 显式给 1）。
   2. **`process.spawn` 的签名是 `(src, name, ppid, uid, gid, argv, opts)`**：漏掉 `gid` 时那个
...
#### 抢占式调度可行性（`debug.sethook` 计数钩子，真机实测电脑 #3）

用户提的方案：**别在每条命令里塞 `msleep`，改用 `debug.sethook` 的计数钩子做抢占**。
`scripts/preempt_probe.ko`（临时载荷）+ `tools/realmachine.py` 注入路径量出来的结论：**可行**。

| 测的东西 | 结果（电脑 #3） |
|---|---|
| 计数钩子会不会触发 | 会（`_VERSION=Lua 5.2`，计数单位≈VM 指令） |
| 钩子里 `coroutine.yield` | **可以**：协程真的挂起（`status=suspended`），调度器 resume 后照常接着跑 |
| 钩子在 `pcall` 里让出 | 可以（工具里到处是 pcall，必须在乎这条） |
| 钩子在 `table.sort` 比较器 / `string.gsub` 回调里让出 | 可以（Cobalt 支持"跨 C/Java 边界让出"） |
| 钩子在**内核 syscall 里**让出 | 可以（`vfs.fs.list/attributes` 循环里一路 resume 5000 次）——**也就是说内核临界区默认拦不住抢占** |
| 钩子成本 | ≈3.3 µs / 次（count=500）；`count=5000` 时相对纯计算 ≈1%（150k 次循环 11ms → 13ms） |
| 粒度 | `count=2000~5000` 已能到 **≈0.1–0.5 ms** 一次（对比现状：一轮调度 8–23 ms、输入延迟 33–133 ms） |
| `debug.sethook(thread, f, "", n)` 线程形式 | 支持（可以在协程还没跑之前就把钩子装上） |
| 行钩子 `"l"` | 支持 |
| 子协程是否继承钩子 | **不继承**（`coroutine.create` 出来的子协程没钩子 → `parallel` 里的活儿抢不到） |

需要配套解决的两件事（设计取舍，见下节「修复方向」）：
1. **内核态屏蔽**：实测 J 证明钩子会在 syscall 内部触发。Delin 的内核是**协作式原子**的
   （ext2 的块分配/位图/目录项更新、pipe/FIFO 记账、进程表都是一串不可中断的步骤），
   被抢占后另一个进程进来做同类操作就可能把元数据写坏。要么给 syscall/VFS 门面/tty 句柄三处
   入口加"内核态深度"计数、深度>0 时钩子只记标志不让出；要么先只抢占"用户态"。
2. **调度器要有 run queue**：现在"每个事件唤醒所有 `filter=nil` 的进程"这套是**事件驱动**的，
   被抢占的进程要等下一个事件（空闲时靠 20Hz 心跳，最坏 50ms）才能回来。抢占要真正降延迟，
   得把 resume 改成"可运行队列 + 每进程一个时间片"的轮转，事件只负责把阻塞的进程标回可运行。

#### 抢占式调度原型（`debug.sethook` 计数钩子 + run queue + 内核锁）

用户提的方案，已落地为**默认关闭**的原型（开关见下）。三块：

| 文件 | 干什么 |
|---|---|
| `src/kernel/lock.lua` | 内核临界区锁：进程进内核(`syscalls`/VFS 门面/tty 与文件句柄)时 `lock.enter()`，出来 `leave()`；**持锁时切片钩子不让出**（那一段是临界区）；内核里阻塞等事件的地方（tty 读、管道读写、FIFO open、`proc.wait`、/dev/kmsg、/dev/random、devdisk fsck 的 yield 回调、md 写、HSE 等待）一律 `lock.pause(fn)` —— **放锁再等、醒来拿回**，否则持锁阻塞会把锁焊死 |
| `src/kernel/scheduler.lua` | 抢占模式：每个进程协程挂 `debug.sethook(hook, "", 5000)`（≈0.1~0.5ms 一片），钩子里 `coroutine.yield("__preempt")` 把 CPU 还给调度器；调度器维护**可运行队列**按片轮转，每轮最多 `BUDGET_MS=8` 毫秒就回事件泵 |
| 开关 | 根上存在 **`/etc/preempt`** 即开（boot 打印 `scheduler: PREEMPTIVE` / `cooperative`）；运行时也能切：syscall `sched.mode` 取 `"get"/"preempt"/"coop"`（对照实验用） |

**关键实现细节（都是踩出来的）**：

- **钩子必须原样透传事件参数**、**代理表必须把 `self` 换回原表**（否则 CC 原生句柄落进"点号"那一支，
  文件里会出现 `table: 0x...` —— 真机症状是 `/mnt/hdd/parts/manifest` 写成 `table: 6030d5c6`，
  于是 `/dev/sda1` 的分区路径变成 `/6030d5c6`，mkfs/mount/fsck 全找不到文件）；**同一个函数不能叠包装**
  （会一层套一层直到 stack overflow）。三条都锁在 `tools/hosttest.lua` 里。
- **关锁要真的关干净**：抢占关闭时（默认）代理只做 `self` 修正、不进 `pcall`、不碰深度计数 ——
  默认路径的开销与改动前一致。锁的**持有者必是"正在跑的进程"**（持锁不让出 + 阻塞处放锁），
  所以 `enter()` 不需要等待队列：撞上别的 pid 就是不变式被破坏，fail-fast。
- **不让出的事件泵**：预算用完后若可运行队列还有人，就往事件队列里塞一颗 `delin_slice` 再 pull
  （pull 立刻返回、排在真实事件之后，既让出主机又不空等一个游戏刻）。**同一时刻只允许一颗在飞**
  —— 不记账的话我们自己塞的比处理得快，队列越积越长、真实按键被埋在后面：实测延迟 30 秒内
  从 30ms 涨到 963ms。
- 进程被 `coroutine.create` 出来的**子协程不继承钩子**（宿主实测）：`parallel` 里的活儿抢不到，
  `/bin` 工具不用 parallel，先记着。

**真机 A/B（电脑 #3，干净镜像；同一台机器同一次开机，`scripts/sched_probe.ko` 写 `/sched.log`）**：

| 场景 | 合成事件投递延迟(平均) |
|---|---|
| 空闲 + 协作 | 18–50 ms |
| 空闲 + 抢占 | **15–21 ms** |
| **1 个不让出的忙等进程(等价 `du`/`find`) + 抢占** | **16–71 ms(稳定, 不随运行时间恶化)** |
| 3 个"每 20ms 让出一次"的负载 + 抢占 | 26–27 ms |
| 同上 + 协作 | 19–25 ms |
| 对照: 同样的不让出进程 + **协作** | 整机冻结直到 CC 的 watchdog 在 ~9.4s 后把它杀掉 |

也就是说：**"命令霸住整机"这件事被消掉了** —— 不让出的后台命令从"冻结整机 9.4 秒"变成"多几十毫秒延迟"，
而空闲延迟也略微下降；代价是每片一次 `yield`/`resume`（宿主实测钩子 ≈3.3µs/次，`count=5000` 时相对纯计算 ≈1%）。

**用户实测的坑（已修）：`find /` 会让其他终端卡死**。把 `find /` 拆成两半压出来的数据
（`scripts/sched_probe.ko`，抢占模式，电脑 #3）：

| 负载 | 事件延迟 | 每秒调度轮数 | 读键盘的进程被恢复 |
|---|---|---|---|
| 空闲 | 28–53 ms | 66–79 | 34–55 /s |
| **只走全树（`find` 的 fs 调用，无输出）** | **130–1419 ms** | **1–6** | **1–2 /s（终端死）** |
| 只往 tty 狂写（`find` 的输出） | 8–120 ms | 89–93 | 18–19 /s |
| 两者一起（真正的 `find /`） | 138–1191 ms | 2–7 | 1–2 /s |

**根因**：`debug.sethook` 的计数钩子**只数用户态 VM 指令**，而"密集小内核调用"的进程
（`find`/`ls` 那种 `fs.list`/`fs.attributes` 循环）指令几乎全花在被屏蔽的临界区里 ——
每 N 条指令的窗口都落在核心里被吞掉，于是它**永远抢不到** CPU：一整轮调度能跑 0.5~1 秒，
别的进程（含读键盘的）每秒只被恢复一两次。输出路径不是问题（B 段一直是健康的）。

**修法**：临界区里钩子只**记账**（`lock.markPreempt()`），**出临界区的那一刻立刻让出**
（`lock.wrap` 的包装器在 `leave()` 之后 `coroutine.yield("__preempt")`，此时已经不在内核里，
安全）。于是"每个内核调用之后"都是一个抢占安全点。修完复测：
...
**改动前的基线**上一样 ng，与调度模式无关），`kern.log` 里 **0 条** 错误（没有内核锁 fail-fast、
没有进程死于异常）。⇒ 抢占模式**没有引入回归**。

**两轮都踩到的坑（realmachine.py 的假失败）**：`reboot_and_collect` 的"引导门禁"用
`debugfs cat /var/log/verify.log` **逐字节比较**判断这一轮有没有从磁盘根起来 —— 而宿主机看到的那份
文件经常还是上一轮的内容（CC 侧缓冲 + NFS 缓存，见下条），于是**自检明明跑完了也报"磁盘根没有启动"**。
这两轮的 `=== verify done ===` 都是事后直接 `dump /var/log/*.log` 才看到的。
