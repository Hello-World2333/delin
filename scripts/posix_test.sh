...
#      注: grep 用标准正则(BRE, `-` 在 RE 里是字面量), 模式里别写未转义的元字符。
# ---------------------------------------------------------------
touch -- "$T/--dash.txt"
...
# ---------------------------------------------------------------
# ---------------------------------------------------------------
# 已知缺口(不在自检里断言, 见 for-ai.md「已知缺口」): xargs/find -exec 起**一串**子进程
# 共用同一个 stdout 管道时, 第一个子进程退出就把管道关掉, 后面几个的输出会丢
# (`printf 'a:b:c' | xargs -d: -n1 echo | wc -l` 只数到 1)。
# 这里只验"重定向到文件"这条不受影响的路径。
# ---------------------------------------------------------------
mkdir -p "$T"
printf 'a:b:c' > "$T/xargs_in"
xargs -a "$T/xargs_in" -d: -n1 echo > "$T/xargs_got"
printf 'a\nb\nc\n' > "$T/xargs_want"
chk xargs_multi_to_file cmp -s "$T/xargs_got" "$T/xargs_want"
